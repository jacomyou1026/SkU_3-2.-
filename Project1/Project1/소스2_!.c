...
int main(int argc, char** argv)
{
	...
	while (!feof(filePtr))
	{
		...
		else if (!_strcmpi("end\n", line) | !_strcmpi("end", line))
		{
			if (foundMain)
			{
				int sline;
		...
		else
		{
			...
			else if (!_strcmpi("function", firstword))
			{
				tempNode.type = 2;
				firstword = strtok(NULL, " ");
				tempNode.exp_data = firstword[0];
